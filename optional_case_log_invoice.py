...
# Columns that the LLM may fill with non-castable strings (e.g. "Unbekannt") when it
# cannot extract a value. These are coerced to NULL before the insert so they match
# the DATE / NUMERIC column types in process_mining_cases.
NUMERIC_CASE_COLUMNS = [
    'net_amount', 'tax_amount', 'total_amount',
    'new_total', 'discount_amount', 'extraction_confidence',
]
...
def sanitize_case_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Coerces non-castable LLM values (e.g. 'Unbekannt') to NULL so they match the
    DATE / NUMERIC column types in process_mining_cases. Anything unparseable becomes NaT/NaN,
    which PostgreSQL stores as NULL."""
    if 'invoice_date' in df.columns:
        df['invoice_date'] = pd.to_datetime(df['invoice_date'], errors='coerce').dt.date
    for col in NUMERIC_CASE_COLUMNS:
    ...
    try:
        for chunk in pd.read_sql(query, engine, chunksize=100):
            ...
            if new_cases:
                ...
                # Coerce non-castable LLM values (e.g. "Unbekannt") to NULL before insert
                df_cases = sanitize_case_dataframe(df_cases)
